...
#include "posit32_headers/SinhForCosh_14.h"
...
posit32_t rlibm_cosh_1412(posit32_t x) {
    ...
    unsigned long Index2 = (dX.x & 0x03FFFFFFFFFFFFFFlu) >> 46lu;
    
    const double* sinhCoeff = SinhForCoshCoeffs[Index];
    const double* coshCoeff = CoshForCoshCoeffs[Index2];
    
    // Compute sinh component
    double sinhL = sinhCoeff[1];
    sinhL *= R2;
    sinhL += sinhCoeff[0];
    sinhL *= R;
